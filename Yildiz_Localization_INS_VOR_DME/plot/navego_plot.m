...
minLength_ref = min(length(ref_t), length(ref_var));
ref_t = ref_t(1:minLength_ref);
ref_var = ref_var(1:minLength_ref);
...
if nargin > 7
    ...
    minLength_gnss = min(length(gnss_var), length(gnss_t));
    gnss_t = gnss_t(1:minLength_gnss);
    gnss_var = gnss_var(1:minLength_gnss);
...
minLength_nav = min(length(nav_t), length(nav_var));
nav_t = nav_t(1:minLength_nav);
nav_var = nav_var(1:minLength_nav);
